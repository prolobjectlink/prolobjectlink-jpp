...
public class TreeList<E> extends AbstractList<E> {
	 ...
	 * <p>
	 * This method runs in O(n + log m) time, where m is the size of this list
	 * and n is the size of {@code c}.
	 *
	 * @param c
	 ...
	 * {@link java.util.List}. The nodes don't know the index of the object they
	 * are holding. They do know however their position relative to their parent
	 * node. This allows to calculate the index of a node while traversing the
	 * tree.
	 * <p>
	...
	static class AVLNode<E> {
		 ...
		 *
		 * @param iterator
		 *            an iterator over the collection, which should already
		 *            point to the element at index {@code start} within the
		 *            collection
		 * @param start
		 ...
		 *            absolute position of this node's parent, or 0 if this node
		 *            is the root
		 * @param prev
		 *            the {@code AVLNode} corresponding to element (start - 1)
		 *            of the collection, or null if start is 0
		 * @param next
		 *            the {@code AVLNode} corresponding to element (end + 1) of
		 *            the collection, or null if end is the last element of the
		 *            collection
		 */
		...
		/**
		 * Locate the element with the given index relative to the offset of the
		 * parent of this node.
		 */
		 ...
		 *
		 * @param index
		 *            is the index of the element to be removed relative to the
		 *            position of the parent node of the current node.
		 */
		 ...
		 * efficiently merging the two AVL trees. This operation is destructive
		 * to both trees and runs in O(log(m + n)) time.
		 *
		 * @param otherTree
		...
		private AVLNode<E> addAll(AVLNode<E> otherTree, final int currentSize) {
			...
			if (otherTree.height > height) {
				...
				// STEP 4: Re-balance the tree and recalculate the heights of
				// s's ancestors.
				s = maxNode;
	...
	static class TreeListIterator<E> implements ListIterator<E> {
		...
		public void remove() {
			...
			// remove
			// reset it now: will be retrieved by next call to next()/previous()
			// via nextIndex
			next = null;
			current = null;
			currentIndex = -1;
